...
var Mime = class {
  ...
  getType(path14) {
    if (typeof path14 !== "string")
      return null;
    const last = path14.replace(/^.*[/\\]/s, "").toLowerCase();
    const ext = last.replace(/^.*\./s, "").toLowerCase();
    const hasPath = last.length < path14.length;
...
function withTrailingSlash(path14) {
...
function createHotChannel(webSocketContainer) {
  ...
  const client = {
    send(payload) {
      ...
      assert2(webSocket, webSocketUndefinedError);
...
var CloudflareDevEnvironment = class extends vite.DevEnvironment {
  ...
  async initRunner(worker, workerConfig) {
    ...
    assert2(webSocket, "Failed to establish WebSocket");
    webSocket.accept();
...
function createCloudflareEnvironmentOptions({
  ...
  environmentName,
  isEntryWorker,
  hasNodeJsCompat: hasNodeJsCompat2
}) {
  const define = getProcessEnvReplacements(hasNodeJsCompat2, mode);
  return {
    resolve: {
...
function getProcessEnvReplacements(hasNodeJsCompat2, mode) {
  ...
  return hasNodeJsCompat2 ? nodeEnvReplacements : {
    ...nodeEnvReplacements,
    "process.env": "{}",
    "global.process.env": "{}",
  ...
  return containersConfig.map((container) => {
    if (isDockerfile(container.image, configPath)) {
      return {
        ...
        image_build_context: container.image_build_context ?? path5.dirname(container.image),
...
function hasLocalDevVarsFileChanged({
  ...
  return [...configPaths].some((configPath) => {
    ...
    return [
      ...
      (localDevFile) => changedFilePath === path7.join(configDir, localDevFile)
    );
  });
}
  ...
  const assetWorkers = [
    ...
    },
    {
      ...
      modules: [
        {
          type: "ESModule",
          path: path8.join(miniflareModulesRoot, VITE_PROXY_WORKER_PATH),
          contents: fs3.readFileSync(
...
import fs4, { realpathSync, statSync as statSync2, promises } from "node:fs";
...
import path9, { dirname as dirname6 } from "node:path";
...
function getMessage(key, parameters, self) {
  ...
  assert7(message !== void 0, "expected `message` to be found");
...
function read(jsonPath, { base, specifier }) {
  ...
  let string;
  try {
    string = fs4.readFileSync(path9.toNamespacedPath(jsonPath), "utf8");
  } catch (error) {
    const exception = (
      /** @type {ErrnoException} */
      error
...
function moduleResolve(specifier, base, conditions, preserveSymlinks) {
  ...
  assert7(resolved !== void 0, "expected to be defined");
...
var NodeJsCompat = class {
  ...
  #getInjects() {
    ...
    for (const [injectedName, moduleSpecifier] of Object.entries(
      ...
      assert8(injects, `expected injects for "${module}" to be defined`);
  ...
  getGlobalVirtualModule(source) {
    ...
    return [
      ...
      ...injects.map(
        ({ injectedName, importName }) => `globalThis.${injectedName} = ${importName};`
      )
    ].join("\n");
  }
  /**
...
`;
      this.sources.forEach((importers, source) => {
        importers.forEach((importer) => {
          message += ` - "${source}" imported from "${path10.relative(this.resolvedViteConfig.root, importer)}"
`;
        });
      });
...
function resolvePluginConfig(pluginConfig, userConfig, viteEnv) {
  ...
  const { CLOUDFLARE_ENV: cloudflareEnv } = vite4.loadEnv(
    viteEnv.mode,
    root,
    /* prefixes */
    ""
  ...
  const nodeJsCompatMap = new Map(
    Object.entries(workers).filter(([_, workerConfig]) => hasNodeJsCompat(workerConfig)).map(([environmentName, workerConfig]) => [
      environmentName,
                ...
                ([environmentName, workerConfig]) => {
                  return [
                    ...
                    createCloudflareEnvironmentOptions({
                      ...
                      hasNodeJsCompat: getNodeJsCompat(environmentName) !== void 0
                    })
                  ];
                }
              )
...
`
    ...
    // Plugin to support additional modules
    {
      ...
      async renderChunk(code, chunk) {
        ...
        for (const match of matches) {
          ...
          const relativePath = vite5.normalizePath(
    ...
    // Plugin that can provide Node.js compatibility support for Vite Environments that are hosted in Cloudflare Workers.
    {
      ...
      applyToEnvironment(environment) {
        return getNodeJsCompat(environment.name) !== void 0;
      },
...
export default mod.default ?? {};
    ...
    // Plugin that handles Node.js Async Local Storage (ALS) compatibility support for Vite Environments that are hosted in Cloudflare Workers.
    {
      ...
      configEnvironment(name) {
        if (hasNodeJsAls(getWorkerConfig2(name))) {
          return {
            resolve: {
                    ...
                    setup(build) {
                      build.onResolve(
                        ...
                        ({ path: path14, importer }) => {
                          if (hasNodeJsAls(workerConfig) && isNodeAlsModule(path14)) {
                            return;
                          }
  ...
  function getNodeJsCompat(environmentName) {
    return resolvedPluginConfig.type === "workers" ? resolvedPluginConfig.nodeJsCompatMap.get(environmentName) : void 0;
  }
}
export {
  cloudflare
