...
const UploadSalaryChartSchema = z.object({
...
async function getCurrentUserFromSession(c) {
...
async function logConnection(c, email, success) {
...
const employeeMiddleware = async (c, next) => {
  const user = await getCurrentUserFromSession(c);
  if (!user) {
    return c.json({ error: "Non authentifié" }, 401);
  }
...
app.post("/api/admin/employees", adminMiddleware, zValidator("json", CreateUserSchema), async (c) => {
  ...
  await c.env.DB.prepare(
    ...
    data.department || "",
    data.position || "",
    data.hire_date || (/* @__PURE__ */ new Date()).toISOString().split("T")[0],
    true
  ).run();
  const { results } = await c.env.DB.prepare(
...
app.post("/api/admin/events", adminMiddleware, async (c) => {
  const body = await c.req.json();
  const currentUser = c.get("currentUser");
  const { title: title2, description, event_date, location } = body;
  await c.env.DB.prepare(
    "INSERT INTO events (title, description, event_date, location, created_by) VALUES (?, ?, ?, ?, ?)"
  ).bind(title2, description, event_date, location, currentUser.email).run();
  const { results } = await c.env.DB.prepare(
...
app.put("/api/admin/events/:id", adminMiddleware, async (c) => {
  ...
  const body = await c.req.json();
  const { title: title2, description, event_date, location, is_active } = body;
  await c.env.DB.prepare(
    "UPDATE events SET title = ?, description = ?, event_date = ?, location = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
  ).bind(title2, description, event_date, location, is_active, id).run();
  const { results } = await c.env.DB.prepare(
...
app.post("/api/admin/announcements", adminMiddleware, async (c) => {
  const body = await c.req.json();
  const currentUser = c.get("currentUser");
  const { title: title2, content, priority } = body;
  await c.env.DB.prepare(
    "INSERT INTO announcements (title, content, priority, created_by) VALUES (?, ?, ?, ?)"
  ).bind(title2, content, priority || "normal", currentUser.email).run();
  const { results } = await c.env.DB.prepare(
...
app.put("/api/admin/announcements/:id", adminMiddleware, async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json();
  const { title: title2, content, priority, is_active } = body;
  await c.env.DB.prepare(
    "UPDATE announcements SET title = ?, content = ?, priority = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?"
  ).bind(title2, content, priority, is_active, id).run();
  const { results } = await c.env.DB.prepare(
...
app.post("/api/admin/documents", adminMiddleware, async (c) => {
  const body = await c.req.json();
  const currentUser = c.get("currentUser");
  const { title: title2, description, document_type } = body;
  ...
  ).bind(title2, description, fileName, fileUrl, document_type, currentUser.email).run();
  const { results } = await c.env.DB.prepare(
...
app.post("/api/employee/requests", employeeMiddleware, async (c) => {
  const body = await c.req.json();
  const user = c.get("currentUser");
  const { request_type, title: title2, description, request_data } = body;
  await c.env.DB.prepare(
    "INSERT INTO employee_requests (user_id, request_type, title, description, request_data) VALUES (?, ?, ?, ?, ?)"
  ).bind(user.id, request_type, title2, description, JSON.stringify(request_data || {})).run();
  const { results } = await c.env.DB.prepare(
